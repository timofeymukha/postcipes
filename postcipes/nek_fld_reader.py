...
class NekInterpolatedData:
    def __init__(self, fname):
        self.data = self.read_int_fld(fname)

    def __getitem__(self, item):
        return self.data.pset[item]

    def read_int(self, infile,emode,nvar):
        """read integer array"""
        isize = 4
    ...
    def read_flt(self, infile,emode,wdsize,nvar):
        """read real array"""
        if (wdsize == 4):
            realtype = 'f'
        elif (wdsize == 8):
    ...
    def read_int_fld(self, fname):
        ...
        ren = self.read_flt(infile,emode,wdsize,1)
        bsize = self.read_flt(infile,emode,wdsize,3)
        belem = self.read_int(infile,emode,3)
        pord = self.read_int(infile,emode,3)
        nstat = self.read_int(infile,emode,1)
        nderiv = self.read_int(infile,emode,1)
        stime = self.read_flt(infile,emode,wdsize,4)
        nrec = self.read_int(infile,emode,1)
        itime = self.read_flt(infile,emode,wdsize,1)
        npoints = self.read_int(infile,emode,1)
        ...
        for il in range(data.npoints):
            lpos = self.read_flt(infile,emode,wdsize,data.ldim)
            lptn = data.pset[il]
        ...
        for il in range(data.nstat):
            for jl in range(data.npoints):
                lsts = self.read_flt(infile,emode,wdsize,1)
                lptn = data.pset[jl]
        ...
        for il in range(data.nderiv):
            for jl in range(data.npoints):
                ldrv = self.read_flt(infile,emode,wdsize,1)
                lptn = data.pset[jl]
    ...
    def print_point_data(self, data, il):
